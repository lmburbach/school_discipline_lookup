...
function myVis(results) {
  ...
  let temp_sys_sch = 'Appling County – All Schools'; // change to all
  ...
  // State for summary numbers & percentile plots
  ...
  const enr_container = select("#left-3").append('div')
  ...
  function render() {
    ...
    console.log(4 - d_annual[0]['YEARS'])

    console.log(x_array)
    let xdim = [min(x_array), max(x_array)]
    console.log(xdim);
    ...
    svg.append('text')
      ...
      .text(`${format('.1f')(d_subgroup[0]['%_DISC_POP'])}%`); //add formatting
